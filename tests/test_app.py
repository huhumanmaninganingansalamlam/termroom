...
async def test_https_proxy_uses_forwarded_scheme_for_static_assets(
    ...
    css_url = f'{expected_scheme}://termroom.example.com/static/app.css?v=56'
    script_url = f'{expected_scheme}://termroom.example.com/static/app.js?v=68'
...
async def test_terminal_page_exposes_shell_tabs_for_in_place_switching(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "data-terminal-session-name" in response.text
        assert str(workspace["tmux_session"]) in response.text
...
async def test_remote_connection_status_is_shared_actionable_and_current(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        script = await client.get("/static/app.js?v=68")
...
async def test_settings_menu_exposes_click_only_pwa_install_guidance(
    ...
    assert "/static/app.js?v=68" in korean_page.text
