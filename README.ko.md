...
Termroom은 Content Security Policy, frame, content-type, referrer header를 직접 설정하고,
클라이언트가 gzip을 허용하면 적합한 동적 text 응답도 압축합니다. Proxy는
`Content-Encoding`과 `Vary`를 그대로 전달해야 합니다. 이 host에
`Content-Security-Policy`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`를
한 번 더 추가하지 말고, 공용 proxy template의 해당 지시문을 끄거나 upstream 값을 한
곳에서만 명시적으로 대체하세요. Application 정책은 직접 local HTTP와 HTTPS/WSS reverse
proxy를 모두 지원하며 local URL을 강제로 HTTPS로 바꾸지 않습니다.
